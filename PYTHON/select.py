...
def write_mpi_binaries(file,config):
    ...
    if str(config['flavor']['mpi']) == 'intelmpi' :
        pref = str(config['system']['comp'] )
        file.write('%define tpls_mpicc   {:s}/bin/mpiicx \n'.format(pref ))
        file.write('%define tpls_mpicxx  {:s}/bin/mpiicpc \n'.format(pref))
...
def write_compiler_flags( file, config ):
    ...
    if gpu == 'cuda' :
        ...
        includes += ' -I{:s}/include'.format(cuda)
        ldflags += ' -L{:s}/lib64'.format( cuda )
        rpath += ' -Wl,-rpath,{:s}/lib64'.format( cuda )
    elif gpu == 'rocm' :
